...
#include "sgpl/utility/do_random_walk_indexmap_approximation.hpp"
#include "sgpl/utility/ThreadLocalRandom.hpp"


TEST_CASE("Test do_random_walk_indexmap_approximation length 0") {
  ...
  std::generate_n(
    ...
    [](){ return sgpl::do_random_walk_indexmap_approximation(0); }
  );
  REQUIRE( std::count( std::begin(walks), std::end(walks), 0 ) == 100 );

}

TEST_CASE("Test do_random_walk_indexmap_approximation length 1") {
  ...
  std::generate_n(
    std::back_inserter(walks), 100,
    [](){ return sgpl::do_random_walk_indexmap_approximation(1); }
  );
...
TEST_CASE("Test do_random_walk_indexmap_approximation length 2") {
  ...
  std::generate_n(
    std::back_inserter(walks), 100,
    [](){ return sgpl::do_random_walk_indexmap_approximation(2); }
  );
  REQUIRE( std::all_of(
...
TEST_CASE("Test do_random_walk_indexmap_approximation various lengths") {
  ...
  for (int length : emp::vector<int>{ 2, 9, 10, 20, 100 } ) {
    ...
    std::generate_n(
      std::back_inserter(walks), 100,
      [length](){ return sgpl::do_random_walk_indexmap_approximation(length); }
    );
